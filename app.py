...
# !! Fix for PyInstaller multiprocessing and CPU dispatcher issues
# !! Must be done before any imports that might trigger numpy
...
import matplotlib # Set matplotlib backend before importing
...
class ConfigReader:
    ...
    def read_api_key_from_config(config_path='.api'):
        ...
        # Use the correct path for both dev and executable
        full_path = ConfigReader.get_resource_path(config_path)
        
        if not os.path.exists(full_path):
            raise FileNotFoundError(f"API configuration file '{config_path}' not found at {full_path}")
    ...
    def read_prompt_from_file(prompt_file=None):
        ...
        # Use the correct path for both dev and executable
        full_path = ConfigReader.get_resource_path(prompt_file)
...
# --- ALL PyQt6-Dependent Class Definitions Go Here ---
# Now that QWidget is defined, you can define your classes.
...
def main():
    """Main entry point for the Ink2TeX system tray application"""
...
if __name__ == "__main__":
    # Import multiprocessing here to avoid early numpy conflicts
    import multiprocessing
    
    # Fix multiprocessing issues in PyInstaller
    multiprocessing.freeze_support()
    # Set multiprocessing start method to avoid CPU dispatcher conflicts
    try:
